...
Generic Python project scaffold built on uv, ruff, basedpyright, pytest, prek, and Claude Code hooks. Copy the repo into a new project, wire up the name in `pyproject.toml`, and run `bash scripts/setup.sh`.
...
- **Commits**: Never run `git commit --no-verify`. The PreToolUse hook blocks it; the pre-commit hooks (ruff, basedpyright, prettier, shellcheck, taplo, yamllint, actionlint) are mandatory.
